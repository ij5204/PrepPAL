...
function TodayView({ logs, calorieGoal, macroGoals, pantryItems, pantryCount, expiringCount, fitnessGoal }: {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      {/* 4. Distribution + Goal Recs (2-col) */}
...
export function NutritionPage() {
  ...
  return (
    <div className="px-4 md:px-6 pt-4 pb-24 md:pb-8 max-w-5xl mx-auto">
      {/* Header */}
      <motion.div style={{ marginBottom: 20 }} initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }}>
        <h2 style={{ fontSize: 22, fontWeight: 800, color: 'var(--txt)', fontFamily: 'var(--fd)', letterSpacing: '-0.3px' }}>Nutrition</h2>
        <p style={{ fontSize: 13, color: 'var(--txt3)', marginTop: 3 }}>What am I eating, what does it mean, and how can I improve?</p>
      </motion.div>

      {/* Timeframe tabs */}
      <motion.div style={{ display: 'flex', gap: 4, marginBottom: 24, background: 'var(--surf)', padding: 4, borderRadius: 'var(--rad)', width: 'fit-content' }} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.04 }}>
        {(['today', 'week', 'month'] as Timeframe[]).map(tf => (
          <button key={tf} onClick={() => setTimeframe(tf)} style={{ padding: '7px 18px', borderRadius: 9, border: 'none', cursor: 'pointer', fontSize: 13, fontWeight: 600, transition: 'all 0.15s ease', background: timeframe === tf ? 'var(--acc)' : 'transparent', color: timeframe === tf ? '#0A0A0A' : 'var(--txt2)' }}>
      ...
      {currentLoading ? (
        ...
        <div style={{ textAlign: 'center', padding: '48px 24px' }}>
          ...
          <button onClick={fetchToday} style={{ padding: '8px 16px', background: 'var(--surf2)', border: '1px solid var(--bdr2)', borderRadius: 8, color: 'var(--txt)', cursor: 'pointer', fontSize: 13 }}>Retry</button>
        </div>
      ) : (
        <>
          {timeframe === 'today' && (
            <TodayView logs={todayLogs} calorieGoal={calorieGoal} macroGoals={macroGoals} pantryItems={pantryItems} pantryCount={pantryCount} expiringCount={expiringCount} fitnessGoal={fitnessGoal} />
          )}
          {timeframe === 'week' && (
